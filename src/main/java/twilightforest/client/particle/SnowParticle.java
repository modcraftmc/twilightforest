...
public class SnowParticle extends TextureSheetParticle {
	...
	public void tick() {
		...
		this.zd *= 0.7D;
		this.yd -= 0.02D;
		this.xd += Mth.sin(0.01F);
		this.zd -= Mth.sin(0.01F);

		if (this.random.nextInt(5) == 0) {
			this.xd *= this.random.nextIntBetweenInclusive(-1, 1);
			this.zd *= this.random.nextIntBetweenInclusive(-1, 1);
		}
	...
	@OnlyIn(Dist.CLIENT)
	public record FallingSnowFactory(SpriteSet sprite) implements ParticleProvider<SimpleParticleType> {
